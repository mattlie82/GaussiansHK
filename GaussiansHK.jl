...
# ╔═╡ 291d1022-0654-11ef-1d20-a94e56f05af2
begin
	using LinearAlgebra
...
# ╔═╡ 69243898-1e27-4e67-8a7e-e953f7fb7d9d
let
md"""
# HK-Gaussians

This notebook computes Gaussian solutions to the Hellinger-Kantorovich (HK) flow problem for the relative Boltzmann entropy. HK-flows are solutions to the problem $\dot\rho = \alpha \mathrm{div}(\rho\nabla \frac{\delta \mathcal{E}}{\delta \rho})-\beta\rho\frac{\delta \mathcal{E}}{\delta \rho}$. Here we consider the relative Boltzmann entropy $\mathcal{E}(\rho) = \int \lambda_\mathrm{B}(\frac{\rho}{\pi})\pi\mathrm{d}x$, where $\lambda_\mathrm{B}(z)=z\log z -z +1$ and $\pi$ is a reference density.

We assume that $\pi$ is a Gaussian and for $\bar q=(\bar A,\bar b,\bar c)$can be written as 
```math
\begin{equation}
\pi(x) = \exp(B_{\bar q}(x)),\quad B_{\bar q}(x) = \bar c+ \bar b\cdot x -\frac{1}{2}x\cdot \bar A x.
\end{equation}
```
If $\rho(0)$ is also a Gaussian with $q_0=(A_0,b_0,c_0)$, we compute the Gaussian solution
$\rho(t,x) = \exp(B_{q(t)}(x))$. In particular, $q(t)$ satisfies the ODE
```math
\begin{align}
\dot A &=\alpha(A\bar A+ \bar A A- A^2) +\beta(\bar A{-}A),\\
\dot b &=\alpha(\bar A b + A\bar b−2Ab)+\beta(\bar b{−}b),\\
\dot c &= \alpha \big((b{-}\bar b)\cdot b +\mathrm{tr}(\bar A{-}A)\big) +\beta(\bar c{-}c).
\end{align}
```
"""
end

# ╔═╡ 49f57261-7e96-4da2-8e1a-99e38926d155
md"""
## Parameters
 - coefficient for transport part $\alpha\geq 0$
 - coefficient for reaction part $\beta\geq 0$
 - time horizon $T>0$
 - maximal step size for integrator $\delta t>0$
"""

# ╔═╡ 496aec55-abab-4724-b8ce-acc2388553b6
# parameters
begin
	α  = 1.0  # coefficient for transport part
	β  = 0.0    # coefficient for reaction part
	T  = 10.0 # time horizon
	dt = T/1000  # maximal step size for integrator
	nothing
end

# ╔═╡ edcedc61-56a3-42cf-84ba-c97ae31ee789
# the function that defines the ODE
function F!(du,u,p,t)
	# extract parameters
	Ufin  = p.x[1] # reference quantities
	alpha = p.x[2]
	beta  = p.x[3]
	# easier access to components
	Afin = Ufin.x[1]; bfin = Ufin.x[2]; cfin = Ufin.x[3]
	A    = u.x[1];	  b    = u.x[2];    c    = u.x[3]
	# time derivatives
	dA = du.x[1]
	db = du.x[2]
	dc = du.x[3]
	# rhs of ODE
	dA .= alpha*(Afin*A + A*Afin - 2A*A)    + beta*(Afin - A)
	db .= alpha*(Afin*b + A*bfin − 2*A*b)     + beta*(bfin − b)
	dc[1] = alpha*(tr(Afin-A) + dot(b,b-bfin)) + beta*(cfin[1] - c[1])
end

# ╔═╡ 9dabf8f4-dab7-42bb-89e1-9848e48c6d21
# Formula for Gaussian, returns function in x
function Gaussian(A,b,c)
	g(x) = exp(c[1] + dot(b,x) - 0.5*dot(x,A*x))
	g
end

# ╔═╡ e9dc8113-c7f2-4f0d-b807-d34967fc98f2
# Initial conditions
begin
	Aini = [1.0 0.0; 0.0 1.0]
	bini = [-1.0 ;-1.0]
	cini = [1.0]
	Uini = ArrayPartition(Aini,bini,cini)
end

# ╔═╡ 0528a7aa-83dc-4a76-b802-8d48af047356
# final conditions / reference state
begin
	Afin = [4.0 2.0; 2.0 2.0]
	bfin = [4.0; 4.0]
	cfin = [2.0]
	Ufin = ArrayPartition(Afin,bfin,cfin)
end

# ╔═╡ 7995d509-9586-4de9-ab9d-be169e95c1fa
#parameters
para = ArrayPartition(Ufin,α,β)

# ╔═╡ 0793adce-3382-41bc-8670-fdaf3d69b0dd
prob=ODEProblem(F!, Uini,(0.0,T), p=para)

# ╔═╡ 03da7611-c4e6-4f68-a5bb-1e545a253d7a
solution = solve(prob, dense=true,dtmax=dt);

# ╔═╡ 6051374b-3f09-4c28-aa53-afd2a09ca7e7
@bind t Slider(0.0:dt:T)

# ╔═╡ cae72235-8d2c-4df6-96fa-1f122af651b5
begin 
	t
	clf()
	u = solution(t)
	...
	X = range(X0, X1, 100)
	Y = range(Y0, Y1, 100)
	# Get the Gaussians as functions of X
	g    = Gaussian(A,b,c)          # Gaussian for 0 < t < T
	# get values 
	GXY    = [g([X[i]; Y[j]]) for j=1:length(Y), i=1:length(X)]

	# set fixed isolines
	GMin = 0.0
	GMax = 20.0
	number_of_isolines = 20
	isolines = collect(GMin:(GMax-GMin)/number_of_isolines:GMax)
	# plot
	contourf(X,Y,GXY,levels=isolines)
...
# ╔═╡ e3bdf430-0dde-4bd2-832d-7d20bf248f6d
let
	clf()
	# compute total mass, should be constant for β = 4 (up to numerical noise)
	s = collect(0:dt:T)
	u = solution(s)
	AA = [u[i].x[1] for i =1:length(s)]
	bb = [u[i].x[2] for i =1:length(s)]
	cc = [u[i].x[3][1] for i =1:length(s)]
	
	K = [exp(cc[i]+0.5*dot(inv(AA[i])*bb[i],bb[i]))/sqrt(det(AA[i])) for i = 1:length(s)]
	plot(s,K)
	gcf()
end

# ╔═╡ 16c78728-5b64-4bd9-8667-08d3da4e7ab2
let
	## compute trajectory of mean $A^{-1}b$
	clf()
	s = collect(0:dt:T)
	u = solution(s)
	AA = [u[i].x[1] for i =1:length(s)]
	bb = [u[i].x[2] for i =1:length(s)]
	cc = [u[i].x[3][1] for i =1:length(s)]
	X = [inv(AA[i])*bb[i] for i = 1:length(s)]
	x = [X[i][1] for i = 1:length(s)]
	y = [X[i][2] for i = 1:length(s)]
	plot(x,y)
	ax = gca()
...
manifest_format = "2.0"
project_hash = "54d882c176e17c0e6fc58127b2af542b959438b1"
